...
# Including the ability from chainer.functions.sum(...)^^^chainersum^^^ and functions/methods in
...
class ArbitraryLoss(object):
    ...
    def __call__(self, toCompare, toClassify, predictWith, aggregate=True):
        ...
        for l in self.standardLossArgs:
            ...
            # ^^^chainermeansquarederror^^^, possibly done because
            # chainer.Classifier^^^chainerclassifier^^^ does it.
...
# This creates adversarial examples to use for training as described in ^^^towards^^^
class AdversarialTrainingConverter(chainer.dataset.Converter):
    ...
    def __call__(self, batch, memoryOf):
        ...
        if self.attack == attacks.FGSM:
            ...
            # ^^^towards^^^ except that, for "step", we round as we are assuming integer-valued
