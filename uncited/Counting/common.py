...
# LabeledImageDataset^^^labeledimagedataset^^^. As suggested by David Crandall^^^graphing^^^, we
...
# SoftPlus^^^softplus^^^ or ModifiedSoftPlus, whose derivatives is pretty flat most of the time).
...
def getEmptyModel(network, activation, base, renormalize, predictionSize):
    ...
    if activation == "relu": # Using ReLU ^^^relu^^^
        f = chainer.functions.relu
    if activation == "sigmoid":
        f = chainer.functions.sigmoid
    if activation == "softplus": # SoftPlus^^^softplus^^^
