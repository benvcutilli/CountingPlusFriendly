...
# Python "math" module[7c779f]
import math

# The collections module, found at [658323]
import collections

import common


# A. This function follows the semantics of how Chainer wraps[39e3c0, softplus(...) function][b72fa5, maybe functions under "Activation functions"] (and maybe
#    [71bd98]) its creation of instances of subclasses of FunctionNode[45a312]
...
#    "Variable"s[d1423b] (which is probably one reason why Chainer does it with its
...
#       However, wrapping was desired (probably also a motivation for [39e3c0, softplus(...) function][b72fa5, maybe functions under "Activation functions"]/
#    [71bd98]) mostly to avoid a bug where calling .apply(...) several times each with a
#    different variable in the tuple results in Chainer throwing an error in [96f1da];
...
#    issue because of the creation of the subclass of FunctionNode[45a312] that
#    [39e3c0, softplus(...) function][b72fa5, maybe functions under "Activation functions"] or [71bd98] does. Plus, using FunctionNode.apply(...) repeatedly
#    is said to not work by [c3e7d9, first part of "Implementing New-Style Functions"]. We therefore duplicate the ModifiedSoftPlus
...
#    layers[16c481] and probably Chainer's Link[307958]. However, my
#    implementation of that required the instance to copy itself to avoid calling apply(...)
#    multiple times with the same instance. This is an issue because [c72348] notes that
...
# Softplus[cf158f], but using a different base during exponentiation and the logarithm. Made to
...
def pureChainerMSP(alternate, potentials):
    ...
    # [faa580] (although I had forgotten what it was). Instead of using
    # Numpy[3873b6], however, we are using Chainer.
...
class ModifiedSoftPlusDerivative(chainer.FunctionNode):
    ...
    # kernel[554041, see 2.1] is defined in here
    def forward_gpu(self, potentialsTuple):
        # CUDA C++[554041, throughout] code:
...
class ModifiedSoftPlus(chainer.FunctionNode):
    ...
    def forward_gpu(self, potentialsTuple):
        # Putting the kernel definition in here is an idea from [c3e7d9, "Write an Elementwise Kernel Function"]. Although not a
        # solution proposed by that reference for this specific problem (at least that I can
        # remember), this ended up being convenient in case there is no CUDA[976898] compiler on
        # the system to compile the kernel, so putting it in __init__(...) wouldn't make sense as
        # all systems would run it. Also, this code is CUDA C++[554041, throughout].
    ...
    # [938dad, equations 69, 74, 75] showed me, not literally, that elements in matrices aren't any
...
# My version of Batch Normalization[867913] instead of using the Chainer
# variant[f5ff13]. This modifies the technique of Chainer's implementation of
...
# [f5ff13] does, and I find that really convenient. As in
...
class BatchNormalizationCustom(chainer.Link):
    ...
    def __init__(self, n):
        ...
        with self.init_scope():
            # "Gamma" and "Beta" from [867913]. Waiting until the first use
            # of this layer to generate the parameters in these variables is what
            # [442c02] does (or some other class in Chainer, possibly
            # [1e9156]), so this may be why I do this here (code not copied from
    ...
    def forward(self, potentials):
        ...
        # [442c02] and/or [1e9156]; using an if statement
...
# ReLU[bffa08] activations in the forward(...) function. However, this appeared to have presented
# a problem, as it was impossible to calculate the second derivative (since the Chainer
# implementation of ReLU[481502] likely has the gradient piecewise 0 and 1 the way I was
# using it -- I can't remember where I heard of this technique[71bd98]), then the second
...
# chosen instead. I initially tried SoftPlus[cf158f], but that was giving me exceptions
...
#   Usage of batch normalization[867913] was recommended by [16ccf1],
...
# Renormalization[e1d64c]. Using [7ca151] defaults for
...
# it is meant to be similar to [b3cf40]'s InceptionV3[2c77d4]-based network (the last
...
class Convolutional(chainer.Chain):
    ...
    def __init__(self,
        ...
        with self.init_scope():
            ...
            # [e1d64c] suggests starting out with those; however, I am not
    ...
    def forward(self, volume):
        ...
        # Don't quite understand why they recommend it, but [867913, § 3.2] says
        ...
        # normalization in this order[867913, § 3.2]).
        ############################################################################################
        #                                                                                          #
        ...
        # count, so that is why ReLU[bffa08] is chosen here
...
class ConvolutionalLarge(chainer.Chain):
    ...
    def __init__(self,
        ...
        with self.init_scope():
            ...
            # number of layers used was chosen because David Crandall[5b1f25]
