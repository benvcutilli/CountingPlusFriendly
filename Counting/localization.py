...
# strong correlation (determined by the values of M' meeting a threshold) between a location in M
# and a location in M' means that they are locations where parts of the same shape is. This is
# because it could be the case that a change in value of pixel p means that the saliency of p' needs
# to be reduced or increased so that the shape count remains the same (or approximately the same) as
# the network would hopefully compensate for the modified pixel. Note that the thresholding occurs
# because it is likely that too many pixels have some correlation with the saliency value, meaning
# that all of them might be accidentally considered in a shape if we don't weed out the pixels that
# don't affect the saliency much. Performing this backpropogation from the single saliency value to
# all pixels *not already attributed to another value on the saliency map* should give us an idea of
# the subset of the remaining pixels that belong to a shape.
...
# "chainer", "PIL", "pathlib", "math", and "argparse" are from ^^^chainer^^^, ^^^pillow^^^,
