...
# chainer.iterators.MultiprocessIterator^^^chainermultiprocessiterator^^^ appears to have an issue
# with loading pieces of the dataset quickly enough.  I could be wrong, but I think
# MultiprocessIterator^^^multiprocessiteratorslownsysresults^^^ synchronizes all processes that are
...
# definition. This does the same thing as maybe ^^^chainermultiprocessiterator^^^/PyTorch's
# DataLoader^^^pytorchdataloader^^^ where a list of indices (self.indices) are shuffled instead of
...
#   As is likely case with ^^^chainermultiprocessiterator^^^ (and also its rationale), it doesn't
...
# ^^^chaineriterator^^^ and just general common practice, the last batch in an epoch will be short
...
class FreeFlowIterator(chainer.dataset.iterator.Iterator):
    ...
    # ^^^chainermultiprocessmultithreaded^^^. "workers" takes the same name and role as its
    ...
    def __init__(self, workers, randomize, dataset, sampleCount, desyncLimit=10):
        ...
        # bottlenecks, an idea from ^^^chainermultiprocessmultithreaded^^^ and
        # ^^^pytorchdataloader^^^. Runaway memory usage was also likely a rationale for them, which
        ...
        # Required attributes^^^chainergithubiterator^^^ for a subclass of
        ...
        # parameter for ^^^chainermultiprocessmultithreaded^^^
